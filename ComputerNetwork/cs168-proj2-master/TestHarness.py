...
def tests_to_run(forwarder):
    from tests import BasicTest, RandomDropTest, SackRandomDropTest, DelayTest, CorruptTest, DropStartAckTest, \
        NonAckTest, DropFirstPacketsTest, RandomDuplicateTest, RandomReorderTest, StartAndEndTest
    BasicTest.BasicTest(forwarder, "one_char.txt")
    RandomDropTest.RandomDropTest(forwarder, "one_char.txt")
    DelayTest.DelayTest(forwarder, "one_char.txt")
    CorruptTest.CorruptTest(forwarder, "one_char.txt")
    DropStartAckTest.DropStartAckTest(forwarder, "one_char.txt")
    NonAckTest.NonAckTest(forwarder, "one_char.txt")
    DropFirstPacketsTest.DropFirstPacketsTest(forwarder, "one_char.txt")
    RandomDuplicateTest.RandomDuplicateTest(forwarder, "one_char.txt")
    RandomReorderTest.RandomReorderTest(forwarder, "one_char.txt")
    BasicTest.BasicTest(forwarder, "one_char.txt")
    SackRandomDropTest.SackRandomDropTest(forwarder, "one_char.txt")
...
if __name__ == "__main__":
    ...
    def usage():
        print
        "Forwarder/Test harness for BEARS-TP"
        print
        "-p PORT | --port PORT Base port value (default: 33123)"
        print
        "-s SENDER | --sender SENDER The path to Sender implementation (default: Sender.py)"
        print
        "-r RECEIVER | --receiver RECEIVER The path to the Receiver implementation (default: Receiver.py)"
        print
